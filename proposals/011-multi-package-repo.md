...
**Status:** proposed. Nothing is implemented. The npm scopes and GitHub org in
section 3 are registered.
...
`proposals/`, `tasks/` and `articles/` move under one folder, `notes/`.
...
notes/                proposals/, tasks/, articles/
packages/
...
site/                 private package
    ...
    index.html, games/, demos/, playground/, spike/  (HTML entry points)
    vite.config.ts
...
| Now | Goes to |
| --- | --- |
| `index.html`, `games/`, `demos/`, `playground/`, `spike/` | `site/` |
| `site/nav.css` | `site/src/shared/nav.css` (the old `site/` folder is only this file) |
| `public/`, `vite.config.ts` | `site/` |
| `src/main.ts`, `cabinet/`, `games/`, `demos/`, `playground/` | `site/src/` |
| `src/pixi-mvt-demo/` (the page behind `spike/index.html`) | `site/src/demos/` or next to the spike page |
...
| `proposals/`, `tasks/`, `articles/` | `notes/` |
| `CLAUDE.md` | `.claude/CLAUDE.md` (Claude Code reads it there too) |
| `llms.txt` | Where it is served from: the docs' `public/` folder |
...
- Create `notes/`; move `CLAUDE.md` into `.claude/` and `llms.txt` to where it
  is served.
...
`AGENTS.md`, `README.md`, `llms.txt`, `.github/copilot-instructions.md`, seven
`docs/` files (including the AI-agent skills), two task files and four
scripts. Also update `AGENTS.md`'s project structure and commands table, and
`docs/reference/project-structure.md`. The existing proposals are historical
and keep their old paths; `notes/proposals/README.md` gets one line saying so.
