...
- 2026-09-25: Monitor row renamed `BPF` (bindings per frame). `<List>` now
  ...
  pool benchmark within noise. Proposal 004 (4.4, open question 2) and the `<List>` patterns guide
...
- 2026-09-26: Views decoupled from `DemoModel`: `TankView` and `ToolbarView`
  ...
  (style guide rule added; repo-wide migration tracked in task 017).
...
- 2026-09-26: Archived. All acceptance criteria met. The follow-ups it found
  are in proposal 012 (pass loop, `scaling` benchmark, render groups, GPU
  figure) and task 017 (method-syntax migration).
