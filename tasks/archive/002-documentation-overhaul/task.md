...
Phase 3 (restructure):
- [x] Learn and Topics merged into "Building with MVT" with 9 sidebar groups
- [x] "The Game Loop" page created from merged ticker/architecture-overview
- [x] "What is MVT?" absorbed into Architecture section
- [x] `adding-a-game.md` moved to `src/games/README.md`
- [x] `walkthrough.md` and `next-steps.md` deleted
- [x] Landing page and all cross-links updated

Phase 4 (reactivity):
- [x] Reactivity page written (why polling)
- [x] Change Detection page revised (how to poll)
- [x] Events and Signals page written (alternatives)
- [x] Standalone `reactivity/` section deleted

Phase 5 (new content):
- [x] Presentation State rewritten (planned, then written)
- [x] Animating Transitions written (planned, then written)
- [x] Testing coverage expanded

Phase 6 (rules):
- [x] Architecture rules split: universal in Architecture, repo-specific in Reference
- [x] Rule naming scheme revised and references updated

Phase 7 (polish):
- [x] Update all docs dir/file names
- [x] Cross-links verified across all pages
- [x] AI agent files updated
- [x] Site map updated to reflect final structure
...
- 2026-04-19 - **Phase 6 complete.** Rules revision done:
  ...
  - Build passes clean.
- 2026-04-19 - **Phase 7 complete.** Final polish done:
  - 7.0: Dissolved `learn/` and `topics/` directories. Created content-group
    directories nested under `building-with-mvt/`: simulating-the-world,
    presenting-the-world, reacting-to-changes, adding-visual-polish,
    animating-transitions, iterating-with-confidence, avoiding-pitfalls.
    File renames: game-loop -> the-game-loop, reactivity -> why-polling,
    managing-view-complexity -> taming-complex-views, phase-transitions ->
    phase-based-transitions.
  - 7.1: Fixed 223+ cross-links across 30 files via automated script, then
    manually fixed 7 remaining depth-mismatch links to the-game-loop and
    3 links to architecture/reference from nested subdirectories.
  - 7.2: Updated AGENTS.md (removed stale Topics row), contributing.md
    (section purposes table, diagram inventory), skill-documentation.md
    (section table, page template, cross-link checklist). All references
    now use new nested directory paths.
  - 7.3: Skipped (site-map.md was a planning artifact, not part of docs).
  - VitePress build passes clean with zero dead links.
- 2026-04-19 - **All phases complete. Task done.**
