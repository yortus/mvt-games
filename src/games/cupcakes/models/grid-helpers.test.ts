...
import type { CupcakeCell, CupcakeKind } from './common';
...
function grid(colCount: number, ...kinds: (CupcakeKind | undefined)[]): CupcakeCell[][] {
    const rowCount = Math.ceil(kinds.length / colCount);
    const result: CupcakeCell[][] = [];
...
function kinds(cells: DeepReadonly<CupcakeCell[][]>): (CupcakeKind | undefined)[] {
    const result: (CupcakeKind | undefined)[] = [];
...
describe('fillCellsNoMatches', () => {
    it('fills every cell', () => {
        ...
        let i = 0;
        const allKinds: CupcakeKind[] = ['strawberry', 'chocolate', 'grape', 'blueberry'];
    ...
    it('produces no matches', () => {
        ...
        let i = 0;
        const allKinds: CupcakeKind[] = ['strawberry', 'chocolate', 'grape', 'blueberry', 'mint', 'lemon'];
